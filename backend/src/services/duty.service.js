...
class DutyService {
    ...
    async autoCompletePendingConfirmations() {
        const istNow = getCurrentIST();

        // Window after which an unconfirmed pending-confirmation duty is auto-completed
        const windowHours = parseInt(process.env.PENDING_CONFIRMATION_AUTO_COMPLETE_HOURS) || 24;
        const cutoff = new Date(istNow.getTime() - windowHours * 60 * 60 * 1000);

        const dutiesToComplete = await Duty.find({
            status: 'pending-confirmation',
            pendingConfirmationAt: { $lte: cutoff }
        }).populate('hospital', 'hospitalLegalName currentAddress location user')
            .populate('assignedTo');

        const bulkOps = [];
        const dutiesForNotification = [];

        for (const duty of dutiesToComplete) {
            bulkOps.push({
                updateOne: {
                    filter: { _id: duty._id, status: 'pending-confirmation' },
                    update: {
                        $set: {
                            status: 'completed',
                            completedAt: istNow
                        ...
                        $push: {
                            statusHistory: {
                                ...
                                timestamp: istNow,
                                changedBy: 'system',
                                reason: `Auto-completed — hospital confirmation window (${windowHours}h) expired`
                            }
                        }
                    }
                }
                ...
                for (const duty of dutiesForNotification) {
                    try {
                        ...
                        if (staff && duty.hospital && duty.hospital.user) {
                            ...
                            console.log(`Auto-confirm notification sent for duty ${duty._id}`);
                        }
                    } catch (notifError) {
                        console.error(`Error sending auto-confirm notification for duty ${duty._id}:`, notifError);
                    }
                }
            }
        }
